...
contract DCNTVaultWrapper is Ownable {
  ...
  /// @notice vault token to be distributed to token holders (ierc20)
  address public immutable vaultDistributionToken;
  /// @notice "ticket" token held by user (ierc721)
  address public immutable nftVaultKey;
  /// @notice unlock date when distribution can start happening
  uint256 public immutable unlockDate;
  /// @notice total supply of nftvaultkey
  uint256 public immutable totalKeySupply;
  ...
  constructor(
    ...
    uint256 _unlockDate,
    uint256 _totalKeySupply
  ) {
    vaultDistributionToken = _vaultDistributionTokenAddress;
    nftVaultKey = _nftVaultKeyAddress;
    unlockDate = _unlockDate;
    totalKeySupply = _totalKeySupply;
  }
  ...
  function vaultBalance() public view returns (uint256) {
    return IERC20(vaultDistributionToken).balanceOf(address(this));
  }
  ...
  function _pendingPayment(uint256 numNftVaultKeys, uint256 totalReceived) private view returns (uint256) {
    return (totalReceived * numNftVaultKeys) / totalKeySupply;
  }
  ...
  function claim(address to, uint256 tokenId) external {
    ...
    require(IERC721(nftVaultKey).ownerOf(tokenId) == to, "address does not own token");
    ...
    require(IERC20(vaultDistributionToken).transfer(to, amount), "Transfer failed");
